...
container.innerHTML = `
    <div class="title">Auto Sacrifice</div>

    <a class="smallFancyButton prefButton option ${MOD.auto_sac_enabled ? '' : 'off'}" id="auto-sac-button" onclick="Game.toggle_auto_sac()">AutoSacrifice ${MOD.auto_sac_enabled ? 'ON' : 'OFF'}</a>
    <label>The global on/off switch for the AutoSacrifice mod.</label><br>

    <a class="smallFancyButton prefButton option ${MOD.do_convert ? '' : 'off'}" id="do-convert-button" onclick="Game.toggle_do_convert()">Auto convert ${MOD.do_convert ? 'ON' : 'OFF'}</a>
    <label>Automatically convert the garden; turn off if you want the seeds but not the lumps.</label><br>

    <a class="smallFancyButton prefButton option ${MOD.avoid_cps_buffs ? '' : 'off'}" id="avoid-cps-buffs-button" onclick="Game.toggle_avoid_cps_buffs()">Avoid CpS buffs ${MOD.avoid_cps_buffs ? 'ON' : 'OFF'}</a>
    <label>Skip planting seeds if a CpS buff is active.</label><br>

    <a class="smallFancyButton prefButton option ${MOD.allow_frenzies ? '' : 'off'}" id="allow-frenzies-button" onclick="Game.toggle_allow_frenzies()">Tolerate frenzies ${MOD.allow_frenzies ? 'ON' : 'OFF'}</a>
    <label>Still plant during regular 7x frenzies.</label><br>

    <a class="smallFancyButton prefButton option ${MOD.do_soil_rotation ? '' : 'off'}" id="do-soil-rotation-button" onclick="Game.toggle_do_soil_rotation()">Auto rotate soil ${MOD.do_soil_rotation ? 'ON' : 'OFF'}</a>
    <label>Automatically switch between fertilizers.</label><br>
`;

l('menu').appendChild(container);
